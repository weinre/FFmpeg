...
int av_opt_set_dict2(void *obj, AVDictionary **options, int search_flags)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "av_opt_set_dict2_1 ret:%d", ret);

    while ((t = av_dict_get(*options, "", t, AV_DICT_IGNORE_SUFFIX))) {
        ret = av_opt_set(obj, t->key, t->value, search_flags);
        av_log(NULL, AV_LOG_DEBUG, "av_opt_set_dict2_2 ret:%d", ret);
        if (ret == AVERROR_OPTION_NOT_FOUND) {
            av_log(NULL, AV_LOG_DEBUG, "av_opt_set_dict2_3 ret:%d", ret);
            ret = av_dict_set(&tmp, t->key, t->value, 0);
        }
        if (ret < 0) {
            av_log(NULL, AV_LOG_DEBUG, "av_opt_set_dict2_4 ret:%d", ret);
            av_log(obj, AV_LOG_DEBUG, "Error setting option %s to value %s.\n", t->key, t->value);
            av_dict_free(&tmp);
            return ret;
        }
        ret = 0;
    ...
    *options = tmp;
    av_log(NULL, AV_LOG_DEBUG, "av_opt_set_dict2_5 ret:%d", ret);
    return ret;
}
